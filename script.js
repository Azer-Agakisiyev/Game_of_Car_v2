...
// ============ YADDAŞ (sessionStorage istifadə olunur ki, çıxıb girəndə sıfırlansın) ============
const STORAGE_KEYS = {
  highScore: 'carGame_highScore',
  owned: 'carGame_owned',
  selected: 'carGame_selected'
};
function getHighScore() { return parseInt(sessionStorage.getItem(STORAGE_KEYS.highScore) || '0', 10); }
function setHighScore(val) { sessionStorage.setItem(STORAGE_KEYS.highScore, Math.floor(val)); }
function getOwnedCars() {
  try { return JSON.parse(sessionStorage.getItem(STORAGE_KEYS.owned)) || [0]; }
  catch { return [0]; }
}
function setOwnedCars(arr) { sessionStorage.setItem(STORAGE_KEYS.owned, JSON.stringify(arr)); }
function getSelectedCar() { return parseInt(sessionStorage.getItem(STORAGE_KEYS.selected) || '0', 10); }
function setSelectedCar(id) { sessionStorage.setItem(STORAGE_KEYS.selected, id); }

// ============ MAĞAZA MAŞINLARI (9 maşın, hər birinin öz super gücü) ============
const ABILITY_INFO = {
  none:    { icon: '',   label: '—' },
  ...
  ghost:   { icon: '👻', label: 'Xəyal — 10s toxunulmazlıq' }, // Vizual olaraq 10s yazıldı
...
function activateAbility() {
  ...
  // Düzəliş: Əgər maşın Gizli Kölgədirsə (ghost), müddəti 10 saniyə (10000ms) et, əks halda 5 saniyə (5000ms)
...
// ============ İDARƏETMƏ (DÜZƏLDİLDİ: pointerdown = ani reaksiya) ============
function moveLeft() {
  if (player.lane > 0) {
    player.lane--;
